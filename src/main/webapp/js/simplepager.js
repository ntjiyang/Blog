...
(function($) {
	...
	$.fn.quickPager = function(options) {
		...
		if(pageCounter > 1) {
			...
			$(".pageNav li").addClass("page");
			$("li.currentPage ").removeClass("page").addClass("currentPage");
			$(".pageNav a").live("click", function() {			
				//grab the REL attribute 
				var clickedLink = $(this).attr("rel");
				options.currentPage = clickedLink;
				//remove current current (!) page
				$("li.currentPage").removeClass("currentPage").addClass("page");
				
				//Add current page highlighting
				$("ul.pageNav").find("a[rel='"+clickedLink+"']").parent("li").removeClass("page").addClass("currentPage");
